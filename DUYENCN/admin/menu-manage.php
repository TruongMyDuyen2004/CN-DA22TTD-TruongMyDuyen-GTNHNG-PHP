...
if ($region_filter) {
    if ($region_filter === 'none') {
        $sql .= " AND (m.region IS NULL OR m.region = '')";
    } else {
        $sql .= " AND m.region = ?";
...
// Đã bỏ lọc theo type - chỉ dùng category_id từ database
        ...
        <div class="filter-section">
            ...
            <form method="GET" action="">
                ...
                <div class="filter-grid" style="grid-template-columns: repeat(4, 1fr); margin-top: 1rem;">
                    <div class="filter-group">
                        <label><i class="fas fa-utensils"></i> Danh mục</label>
                    ...
                    <div class="filter-group">
                        <label><i class="fas fa-map-marker-alt"></i> Vùng miền</label>
                        <select name="region">
                            <option value="">-- Tất cả vùng --</option>
                            <option value="mien_bac" <?php echo ($_GET['region'] ?? '') == 'mien_bac' ? 'selected' : ''; ?>>🏔️ Miền Bắc</option>
                            <option value="mien_trung" <?php echo ($_GET['region'] ?? '') == 'mien_trung' ? 'selected' : ''; ?>>🏖️ Miền Trung</option>
                            <option value="mien_nam" <?php echo ($_GET['region'] ?? '') == 'mien_nam' ? 'selected' : ''; ?>>🌴 Miền Nam</option>
                            <option value="quoc_te" <?php echo ($_GET['region'] ?? '') == 'quoc_te' ? 'selected' : ''; ?>>🌍 Quốc tế</option>
                            <option value="none" <?php echo ($_GET['region'] ?? '') == 'none' ? 'selected' : ''; ?>>❓ Chưa phân loại</option>
                        </select>
                    </div>
    ...
    <div id="addModal" class="modal" style="display: none;">
        <div class="modal-content" style="max-width: 600px;">
            ...
            <form id="addForm" method="POST" action="api/add-menu-item.php" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>🗺️ Vùng miền</label>
                    <select name="region" id="add_region">
                        <option value="">-- Chọn vùng miền --</option>
                        <option value="mien_bac">🏔️ Miền Bắc</option>
                        <option value="mien_trung">🏖️ Miền Trung</option>
    ...
    <div id="editModal" class="modal" style="display: none;">
        <div class="modal-content" style="max-width: 600px;">
            ...
            <form id="editForm" method="POST" action="api/update-menu-item.php" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>🗺️ Vùng miền</label>
                    <select name="region" id="edit_region">
                        <option value="">-- Chọn vùng miền --</option>
                        <option value="mien_bac">🏔️ Miền Bắc</option>
                        <option value="mien_trung">🏖️ Miền Trung</option>
                        <option value="mien_nam">🌴 Miền Nam</option>
