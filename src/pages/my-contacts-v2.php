...
if (isset($_SESSION['customer_id'])) {
    ...
    try {
        ...
        if ($customer) {
            ...
            $searched = true; // Tự động tìm kiếm
        }
...
// Tự động tìm kiếm nếu đã có email (từ login hoặc search)
if ($searched && !empty($searchEmail) && filter_var($searchEmail, FILTER_VALIDATE_EMAIL)) {
    try {
        if (!isset($conn)) {
            $db = new Database();
            $conn = $db->connect();
        ...
        // Kiểm tra xem người dùng đã like tin nhắn nào chưa
        if ($isLoggedIn && isset($_SESSION['customer_id'])) {
            ...
            if (!empty($contact_ids)) {
                ...
                $stmt = $conn->prepare("SELECT contact_id FROM contact_reply_likes WHERE customer_id = ? AND contact_id IN ($placeholders)");
                $stmt->execute(array_merge([$_SESSION['customer_id']], $contact_ids));
                $liked_contacts = $stmt->fetchAll(PDO::FETCH_COLUMN);
                
                // Đánh dấu các tin nhắn đã like
                foreach ($contacts as &$contact) {
                    $contact['user_liked'] = in_array($contact['id'], $liked_contacts);
                }
            }
            ...
            if (!empty($contact_ids)) {
                $placeholders = str_repeat('?,', count($contact_ids) - 1) . '?';
                $stmt = $conn->prepare("SELECT contact_id FROM contact_reply_likes WHERE customer_email = ? AND contact_id IN ($placeholders)");
                $stmt->execute(array_merge([$searchEmail], $contact_ids));
                $liked_contacts = $stmt->fetchAll(PDO::FETCH_COLUMN);
...
<section class="my-contacts-section">
    <div class="container">
        ...
        <!-- Form tìm kiếm (chỉ hiện nếu chưa đăng nhập) -->
        <?php if (!$isLoggedIn): ?>
        ...
        <div class="logged-in-notice">
            ...
            <p>Xin chào! Đang hiển thị tin nhắn của email: <strong><?php echo htmlspecialchars($searchEmail); ?></strong></p>
        </div>
        <?php endif; ?>
                            ...
                            <div class="message-bubble admin-message">
                                ...
                                <div class="message-footer">
                                    ...
                                    <div class="message-actions">
                                        <button 
                                            class="btn-like <?php echo ($contact['user_liked'] ?? false) ? 'liked' : ''; ?>" 
                                            onclick="toggleLike(<?php echo $contact['id']; ?>, '<?php echo htmlspecialchars($searchEmail); ?>')"
                                            data-contact-id="<?php echo $contact['id']; ?>"
                                        >
                                            <i class="<?php echo ($contact['user_liked'] ?? false) ? 'fas' : 'far'; ?> fa-heart"></i>
                                            <span class="like-count"><?php echo $contact['likes_count'] ?? 0; ?></span>
                                        </button>
                                    </div>
                                </div>
                            </div>
